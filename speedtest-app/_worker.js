...
function splashHtml() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Linje</title><style>:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;min-height:100svh;display:grid;place-items:center;overflow:hidden;background:radial-gradient(circle at 50% 42%,rgba(255,255,255,.08),transparent 24rem),linear-gradient(180deg,#050608 0%,#090a0d 52%,#030406 100%);color:#f5f5f7;font-family:Inter,Arial,sans-serif;padding:32px}.grid{position:fixed;inset:0;background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px);background-size:72px 72px;mask-image:radial-gradient(circle at center,#000 0%,transparent 74%);animation:drift 22s linear infinite}.mark-wrap{position:relative;width:min(384px,calc(100vw - 48px));aspect-ratio:1;display:grid;place-items:center;filter:drop-shadow(0 32px 90px rgba(0,0,0,.5))}.orbit{position:absolute;border-radius:999px;border:1px dashed rgba(255,255,255,.22)}.outer{inset:0;animation:spin 18s linear infinite}.middle{inset:14%;border-color:rgba(94,160,255,.25);animation:spin 13s linear infinite reverse}.inner{inset:28%;border-color:rgba(168,137,255,.28);animation:spin 9s linear infinite}.mark{width:42%;height:42%;display:grid;place-items:center;border:1px solid rgba(255,255,255,.22);border-radius:8px;background:rgba(255,255,255,.12);box-shadow:0 0 42px rgba(255,255,255,.18)}.mark img{width:52%;height:52%}@keyframes spin{to{transform:rotate(360deg)}}@keyframes drift{to{background-position:72px 72px}}@media(max-width:560px){body{padding:18px}}</style></head><body><div class="grid"></div><main class="mark-wrap" aria-label="Linje"><div class="orbit outer"></div><div class="orbit middle"></div><div class="orbit inner"></div><span class="mark"><img src="/logo/logo-icon.svg" alt=""></span></main></body></html>`;
}
