...
.linje-street-prompt{position:fixed;left:50%;bottom:82px;transform:translateX(-50%);z-index:1002;display:flex;align-items:center;gap:10px;max-width:min(620px,calc(100vw - 24px));padding:10px 12px;border:1px solid rgba(255,255,255,.18);border-radius:14px;background:linear-gradient(145deg,rgba(12,12,14,.7),rgba(5,5,6,.48));box-shadow:inset 0 1px 1px rgba(255,255,255,.18),0 18px 60px rgba(0,0,0,.52);backdrop-filter:blur(28px) saturate(1.25);-webkit-backdrop-filter:blur(28px) saturate(1.25);color:#f4f4f5;font:13px Inter,Arial,sans-serif}
.linje-street-prompt[hidden],.linje-street-panel[hidden]{display:none!important}
.linje-street-dot{width:12px;height:12px;border-radius:999px;background:#fff;box-shadow:0 0 0 8px rgba(59,130,246,.14),0 0 22px rgba(59,130,246,.8);flex:0 0 auto}
.linje-street-copy{display:grid;gap:2px;min-width:0}.linje-street-title{font-weight:800;letter-spacing:0}.linje-street-sub{color:#a1a1aa;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.linje-street-actions{display:flex;gap:8px;margin-left:auto}.linje-street-btn{min-height:32px;padding:7px 10px;border:1px solid rgba(255,255,255,.2);border-radius:10px;background:rgba(255,255,255,.08);color:#fff;font-weight:800;font-size:12px;cursor:pointer}.linje-street-btn:hover{background:rgba(255,255,255,.16);border-color:rgba(255,255,255,.4)}
.linje-street-panel{position:fixed;right:18px;bottom:84px;z-index:1001;width:min(560px,calc(100vw - 32px));height:min(430px,calc(100vh - 132px));overflow:hidden;border:1px solid rgba(255,255,255,.16);border-radius:16px;background:rgba(8,8,10,.72);box-shadow:inset 0 1px 1px rgba(255,255,255,.16),0 24px 80px rgba(0,0,0,.62);backdrop-filter:blur(24px) saturate(1.24);-webkit-backdrop-filter:blur(24px) saturate(1.24);color:#f4f4f5}
.linje-street-head{height:52px;display:flex;align-items:center;gap:10px;padding:0 12px;border-bottom:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.045)}.linje-street-head strong{font-size:14px}.linje-street-coords{color:#a1a1aa;font-size:11px;margin-left:auto;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}.linje-street-close{width:32px;height:32px;border:1px solid rgba(255,255,255,.15);border-radius:9px;background:rgba(255,255,255,.06);color:#fff;cursor:pointer;font-size:18px;line-height:1}
.linje-street-frame{width:100%;height:calc(100% - 52px);border:0;background:#050506}.linje-street-empty{height:calc(100% - 52px);display:grid;place-items:center;padding:24px;text-align:center;color:#a1a1aa;font-size:13px}
...
function floatingViewerPatchScript() {
  return `<style id="linje-floating-viewer-style">
.linje-movable-viewer{position:fixed!important;min-width:280px!important;min-height:180px!important;max-width:calc(100vw - 16px)!important;max-height:calc(100vh - 16px)!important;z-index:1400!important;box-sizing:border-box!important}
.linje-movable-viewer[data-linje-floating-active="true"]{z-index:1401!important;box-shadow:0 28px 82px rgba(0,0,0,.62)!important}
.linje-movable-viewer[data-linje-floating-active="true"] iframe,.linje-movable-viewer[data-linje-floating-active="true"] canvas,.linje-movable-viewer[data-linje-floating-active="true"] video{pointer-events:none!important}
.linje-floating-handle{position:absolute;left:10px;top:8px;width:34px;height:24px;z-index:4;border:1px solid rgba(255,255,255,.16);border-radius:9px;background:rgba(8,8,10,.58);backdrop-filter:blur(18px) saturate(1.2);-webkit-backdrop-filter:blur(18px) saturate(1.2);cursor:grab;touch-action:none;box-shadow:inset 0 1px 1px rgba(255,255,255,.16),0 10px 28px rgba(0,0,0,.35)}
.linje-floating-handle:before{content:"";position:absolute;left:10px;top:7px;width:3px;height:3px;border-radius:50%;background:rgba(255,255,255,.72);box-shadow:7px 0 rgba(255,255,255,.72),14px 0 rgba(255,255,255,.72),0 7px rgba(255,255,255,.5),7px 7px rgba(255,255,255,.5),14px 7px rgba(255,255,255,.5)}
.linje-floating-grip{position:absolute;right:0;bottom:0;width:24px;height:24px;z-index:4;cursor:nwse-resize;touch-action:none}
.linje-floating-grip:after{content:"";position:absolute;right:6px;bottom:6px;width:9px;height:9px;border-right:2px solid rgba(255,255,255,.42);border-bottom:2px solid rgba(255,255,255,.42)}
@media(max-width:760px){.linje-floating-handle{left:8px;top:8px}.linje-movable-viewer{min-width:min(280px,calc(100vw - 16px))!important}}
</style><script>(()=>{const STORE="linje-floating-viewer-layout";const G=8;let active=null;const clamp=(v,min,max)=>Math.min(Math.max(v,min),Math.max(min,max));const vp=()=>({w:innerWidth,h:innerHeight});const load=key=>{try{return JSON.parse(localStorage.getItem(STORE+"-"+key)||"null")}catch{return null}};const save=(key,box)=>{try{localStorage.setItem(STORE+"-"+key,JSON.stringify(box))}catch{}};const keyFor=el=>el.id||[...el.classList].find(c=>/speed|viewer|panel|street/i.test(c))||"tool";const boxOf=el=>{const r=el.getBoundingClientRect();return{x:r.left,y:r.top,w:r.width,h:r.height}};const apply=(el,box)=>{const size=vp();const w=clamp(box.w||560,280,size.w-G*2);const h=clamp(box.h||360,180,size.h-G*2);el.style.left=clamp(box.x??(size.w-w-G),G,size.w-w-G)+"px";el.style.top=clamp(box.y??(size.h-h-84),G,size.h-h-G)+"px";el.style.width=w+"px";el.style.height=h+"px";el.style.right="auto";el.style.bottom="auto"};const candidate=el=>{if(!(el instanceof HTMLElement)||el.classList.contains("linje-movable-viewer"))return false;if(el.closest(".linje-movable-viewer"))return false;if(el.matches(".cesium-viewer,.cesium-widget,.app-shell,.app-shell__globe,.boot-overlay,.auth-globe-frame"))return false;const cls=el.className&&String(el.className);let frame=null;try{frame=el.querySelector&&el.querySelector(":scope > iframe,:scope > div > iframe,:scope > section > iframe")}catch{}const src=frame?.getAttribute("src")||"";return el.classList.contains("linje-street-panel")||/speedtest|speed-test/i.test(cls)||/speedtest|ookla|fast\\.com/i.test(src)};function start(el,mode,ev){ev.preventDefault();ev.stopPropagation();const b=boxOf(el);active={el,mode,key:keyFor(el),x:ev.clientX,y:ev.clientY,box:b};el.dataset.linjeFloatingActive="true";document.body.style.userSelect="none"}function move(ev){if(!active)return;const dx=ev.clientX-active.x,dy=ev.clientY-active.y;const b={...active.box};if(active.mode==="move"){b.x+=dx;b.y+=dy}else{b.w+=dx;b.h+=dy}apply(active.el,b)}function end(){if(!active)return;const a=active;active=null;a.el.dataset.linjeFloatingActive="false";document.body.style.userSelect="";save(a.key,boxOf(a.el))}addEventListener("pointermove",move);addEventListener("pointerup",end);addEventListener("pointercancel",end);function enhance(el){if(!candidate(el))return;el.classList.add("linje-movable-viewer");const key=keyFor(el);apply(el,load(key)||boxOf(el));if(!el.querySelector(":scope > .linje-floating-handle")){const h=document.createElement("button");h.type="button";h.className="linje-floating-handle";h.title="Drag viewer";h.setAttribute("aria-label","Drag viewer");h.addEventListener("pointerdown",ev=>start(el,"move",ev));el.prepend(h)}if(!el.querySelector(":scope > .linje-floating-grip")){const g=document.createElement("span");g.className="linje-floating-grip";g.setAttribute("aria-hidden","true");g.addEventListener("pointerdown",ev=>start(el,"resize",ev));el.append(g)}}function scan(){document.querySelectorAll(".linje-street-panel,.speedtest-viewer,.speedtest-panel,[data-speedtest-viewer],section,div").forEach(enhance)}scan();new MutationObserver(scan).observe(document.documentElement,{childList:true,subtree:true});addEventListener("resize",()=>document.querySelectorAll(".linje-movable-viewer").forEach(el=>apply(el,boxOf(el))))})();</script>`;
}
