...
const ASSET_VERSION = "linje-20260601-12";
...
function rewriteHtml(value, pathname = "") {
  ...
  return branded.replace(
    ...
    `$1="$2?v=${ASSET_VERSION}"`,
  ).replace("</body>", `${pathname === "/auth-background" ? authBackgroundPatch() : ""}${brandPatchScript()}${pathname === "/auth-background" ? "" : streetViewPatchScript()}</body>`);
}
...
function streetViewPatchScriptLegacy() {
  return `<style id="linje-street-style">
