...
const THEME_STYLE = `
  :root {
    --tl-shell-max: 1180px;
    --tl-page-pad: clamp(14px, 2vw, 28px);
    --tl-bg:      #050505;
    --tl-surface: #101010;
    --tl-card:    #181818;
    --tl-border:  #2b2b2b;
  ...
  * { box-sizing: border-box; }
  ...
  .tl-header {
    padding: 16px var(--tl-page-pad) 12px !important;
  }
  .tl-main-content {
    display: grid;
    ...
    gap: 18px;
    align-items: start;
    padding: 18px var(--tl-page-pad) 120px !important;
  ...
  .tl-output-content {
    ...
    gap: 16px;
    align-items: start;
    padding: 18px var(--tl-page-pad) 48px !important;
    overflow: visible !important;
  }
  .tl-output-primary {
    min-width: 0;
  ...
  .tl-generate-bar {
    ...
    max-width: var(--tl-shell-max);
    margin: 0 auto;
    padding: 10px var(--tl-page-pad) 20px;
  ...
  @media (max-width: 900px) {
    ...
    background: linear-gradient(0deg,var(--tl-bg) 60%,transparent);
      border-top: none;
    }
  }
`;
...
const LS = {
  get: (k, d) => {
    if (k === KEYS_KEY) {
      const cookieValue = cookieStore.get(k, null);
  ...
  set: (k, v) => {
    if (k === KEYS_KEY) return cookieStore.set(k, v);
    try { localStorage.setItem(k, JSON.stringify(v)); } catch {}
  },
  remove: (k) => {
    if (k === KEYS_KEY) return cookieStore.remove(k);
...
function SaveDrawer({appState, tunePages, onLoad, onClose}) {
  ...
  return (
    <div className="tl-shell" style={{position:"fixed",inset:0,background:"#000a",zIndex:300,display:"flex",flexDirection:"column",justifyContent:"flex-end",margin:"0 auto"}}>
...
function HamburgerMenu({onClose, onNav, isOutputScreen, appState}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:50,display:"flex",flexDirection:"column",justifyContent:"flex-start"}}>
      ...
      <div className="tl-shell" style={{background:C.bg,borderTop:`1px solid ${C.border}`,width:"100%",margin:"0 auto"}}>
...
IMPORTANT â€” do NOT change these settings: ${lockList.join(", ")}.` : "";
      ...
      {/* Paste-back modal */}
      {showPaste&&(
        <div className="tl-shell" style={{position:"fixed",inset:0,background:"#000c",zIndex:500,display:"flex",alignItems:"flex-end",margin:"0 auto"}}>
      ...
      {/* Tune card */}
      <div className="tl-output-content" style={{flex:1,overflowY:"auto",padding:"10px 14px 120px"}}>
        {aiEnhancing&&(
          <EnhancingBar color={prov.color} icon={prov.icon} label={prov.label}/>
        )}
       {!aiEnhancing&&tunePages._summary&&(
          <div className="tl-full-span" style={{background:"rgba(0,255,133,0.05)",border:"1px solid rgba(0,255,133,0.2)",
        ...
        {data ? (
          <>
            <div className="tl-output-primary">
...
const LOCAL_COLORS = Array.isArray(localColorsData?.colors) && localColorsData.colors.length
  ? localColorsData.colors
  : RAW_FALLBACK;
...
function ForzaColorsInner({accentColor, onCountChange}) {
  ...
  const [colorData, setColorData] = useState(() => LOCAL_COLORS);
  const listRef = useRef(null);
  const isFavSheet = sheet === "Favs";
  const RAW = colorData;

  useEffect(() => {
    if(onCountChange) onCountChange(LOCAL_COLORS.length);
  }, []);
...
function DetailScreen({ entry, onBack, onCopy, copied, isFav, onToggleFav }) {
  ...
  return (
    <div className="tl-shell" style={{ background: C.bg, minHeight: "100vh", margin: "0 auto", fontFamily: C.fBody, display: "flex", flexDirection: "column" }}>
      <style>{FONTS + THEME_STYLE}</style>
    ...
    <div className="tl-shell" style={{minHeight:"100vh",background:C.bg,color:C.text,margin:"0 auto",fontFamily:C.fBody,display:"flex",flexDirection:"column"}}>
      ...
      {overlay==="about" && (
        <div style={{position:"fixed",inset:0,zIndex:50,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"flex-end"}} onClick={()=>setOverlay(null)}>
          <div className="tl-shell" style={{background:C.bg,borderTop:`2px solid ${C.green}30`,borderRadius:"20px 20px 0 0",padding:"20px 16px 32px",width:"100%",margin:"0 auto"}} onClick={e=>e.stopPropagation()}>
...
export default function ForzaTuner() {
  ...
  useEffect(()=>{
    ...
    // Apply class-based theme for the browser page.
    const applyTheme = (e) => {
...
const searchResults = carSearch.length > 0
  ...
  return (
    <div className="tl-shell" style={{minHeight:"100vh",background:C.bg,color:C.text,margin:"0 auto",fontFamily:C.fBody,display:"flex",flexDirection:"column"}}>
      ...
      <div className="tl-main-content" style={{flex:1,overflowY:"auto",padding:"10px 14px 130px"}}>
        <div className="tl-main-stack">
