...
} = useCameraTestRunner(testSources, testStatuses);
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.stats}>
          ...
          <div className={styles.statCard}>
            <h3>Responsive (OK)</h3>
            <p className={styles.badgeOk} style={{ background: 'none', padding: 0, border: 'none' }}>
              {stats.ok}
              {' '}
              <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>
                (
          ...
          <div className={styles.statCard}>
            <h3>Errors / Blocked</h3>
            <p className={styles.badgeError} style={{ background: 'none', padding: 0, border: 'none' }}>
              {stats.error}
              {' '}
              <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>
                (
          ...
          <div className={styles.statCard}>
            <h3>Timeouts</h3>
            <p className={styles.badgeWarn} style={{ background: 'none', padding: 0, border: 'none' }}>
              {stats.timeout}
              {' '}
              <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>
                (
          ...
          <div className={styles.statCard}>
            <h3>Pending</h3>
            <p
              className={stats.testing > 0 ? styles.badgeTesting : ''}
              style={{
 background: 'none', padding: 0, border: 'none', color: stats.testing > 0 ? '#60a5fa' : 'inherit'
}}
            >
              {stats.pending + stats.testing}
              {' '}
              <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>
                (
                {dStats.pending + dStats.testing}
                {' '}
                shown)
              </span>
            </p>
        ...
        <div
          ...
          style={{
 borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1.5rem', marginBottom: '0.5rem', alignItems: 'flex-start'
}}
        >
          ...
          {loading ? (
            <button type="button" className={styles.button} disabled>
              <span>Loading Cameras...</span>
                  ...
                  <button type="button" className={styles.button} onClick={stopTests} style={{ background: '#ef4444' }}>
                    🛑 Stop Tests
                  </button>
                ) : (
                  <button type="button" className={styles.button} onClick={runTests} disabled={cameras.length === 0}>
                    ▶ Run Tests
                  </button>
                )}
        </div>
        ...
        <div className={styles.controls} style={{ alignItems: 'flex-start' }}>
          ...
          <button
            ...
            style={{ background: 'rgba(255,255,255,0.1)', marginLeft: 'auto' }}
            onClick={() => {
