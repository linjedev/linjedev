...
const latestSchema = z.object({
  ...
  provider: z.enum(["cs2.sh", "cs2cap", "pricempire", "skinport", "steam", "csfloat", "c5game", "cspriceapi"]).default("cs2.sh"),
  ...
  if ((payload.provider === "c5game" || payload.provider === "cspriceapi" || payload.provider === "steam" || payload.provider === "csfloat") && (payload.marketHashNames ?? []).length === 0) {
    context.addIssue({
      code: "custom",
