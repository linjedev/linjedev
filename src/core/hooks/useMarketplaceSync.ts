...
function getDemoDefaultPluginIds(): Set<string> | null {
    const envVar = process.env.NEXT_PUBLIC_DEMO_DEFAULT_PLUGINS || "";
    const pluginIds = new Set<string>();
    envVar.split(",").forEach((s) => {
        const clean = s.trim();
        if (clean) pluginIds.add(clean);
    });
    return pluginIds.size > 0 ? pluginIds : null;
}
...
export function useMarketplaceSync(hostReady: boolean) {
    ...
    async function loadManifest(manifest: PluginManifest) {
        ...
        try {
            ...
            if (isDemo) {
                const demoDefaultPlugins = getDemoDefaultPluginIds();
                shouldEnable = demoDefaultPlugins ? demoDefaultPlugins.has(manifest.id) : true;
            }
